...
# Copy source code directly (skip complex deps stage caching to fix build errors)
COPY . .
...
# Fix for Next.js proxying (keep as is)
RUN sed -i -e "s/30000/600000/" \
    "node_modules/.pnpm/next@15.4.7_react-dom@19.1.0_react@19.1.0__react@19.1.0/node_modules/next/dist/server/lib/router-utils/proxy-request.js" \
    "node_modules/.pnpm/next@15.4.7_react-dom@19.1.0_react@19.1.0__react@19.1.0/node_modules/next/dist/esm/server/lib/router-utils/proxy-request.js" || true
...
# Install production dependencies only (prune devDeps)
# We already have node_modules from builder, but we want to prune?
# RUN pnpm prune --prod
# Or just run install --prod again?
# Since we copied node_modules, it has everything.
# Let's trust pnpm install --prod to clean up or just leave it. 
# Re-running install --prod is safer to ensure production only.
